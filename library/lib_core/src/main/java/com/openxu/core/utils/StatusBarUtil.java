...
public class StatusBarUtil {
    ...
    public static void immersive(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        ...
        if (Build.VERSION.SDK_INT >= 21) {   //5.0及以上系统才支持
            ...
            //两个flag同时使用让应用的主体内容占用系统状态栏的空间
            systemUiVisibility |= View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN;
            systemUiVisibility |= View.SYSTEM_UI_FLAG_LAYOUT_STABLE;
            //通过DecorView设置系统UI元素可见性
    ...
    public static void darkMode(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (isFlyme4Later()) {
            ...
            FLog.d("2.-------------当前MIUI6以上："+ Color.BLACK);
            darkModeForMIUI6(window, true);
            immersive(window,color,alpha);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            FLog.d("3.-------------当前Android23以上");
            darkModeForM(window, true);
            immersive(window, color, alpha);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            FLog.d("4.-------------当前Android19以上");
            window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            setTranslucentView((ViewGroup) window.getDecorView(), color, alpha);
        } else {
            FLog.d("5.-------------其他");
...
//            decor.setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE);
    ...
    public static boolean darkModeForMIUI6(Window window, boolean darkmode) {
        ...
        try {
            ...
            FLog.w("MIUI7以上采用android系统API："+(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M));
