...
//        return spanString;
    ...
    public static SpannableString getSpecifyLableValueSpan(String lable, String space, String value, int preTextSize, int afterTextSize, int preColor, int afterColor) {
        lable = TextUtils.isEmpty(lable) ? "" : lable;
        value = TextUtils.isEmpty(value) ? "" : value;
        SpannableString spanString = new SpannableString(lable + space + value);
        ForegroundColorSpan span = new ForegroundColorSpan(preColor);
        ...
        spanString.setSpan(new AbsoluteSizeSpan(afterTextSize), lable.length() + space.length(), lable.length() + space.length() + value.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

        return spanString;
    }


    public static String getNoEmptyStr(String lable) {
        return (TextUtils.isEmpty(lable) || "null".equals(lable))?" ":lable;
    }
}
