...
public final class FBarUtils {
    ...
    public static int getActionBarHeight() {
        ...
        if (FUtils.getApp().getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) {
            return TypedValue.complexToDimensionPixelSize(
                    tv.data, FUtils.getApp().getResources().getDisplayMetrics()
            );
        }
        return 0;
    }
    ...
    private static void invokePanels(final String methodName) {
        try {
            ...
            Object service = FUtils.getApp().getSystemService("statusbar");
    ...
    public static void setNavBarVisibility(@NonNull final Window window, boolean isVisible) {
        ...
        for (int i = 0, count = decorView.getChildCount(); i < count; i++) {
            ...
            if (id != View.NO_ID) {
                String resourceEntryName = FUtils.getApp()
                        .getResources()
    ...
    public static boolean isSupportNavBar() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            WindowManager wm = (WindowManager) FUtils.getApp().getSystemService(Context.WINDOW_SERVICE);
        ...
        boolean menu = ViewConfiguration.get(FUtils.getApp()).hasPermanentMenuKey();
