...
public final class FUtils {
    ...
    static Context getTopActivityOrApp() {
        if (isAppForeground()) {
            ...
            return topActivity == null ? FUtils.getApp() : topActivity;
        } else {
    ...
    static boolean isAppForeground() {
        ActivityManager am = (ActivityManager) FUtils.getApp().getSystemService(Context.ACTIVITY_SERVICE);
        if (am == null) return false;
        List<ActivityManager.RunningAppProcessInfo> info = am.getRunningAppProcesses();
        if (info == null || info.size() == 0) return false;
        for (ActivityManager.RunningAppProcessInfo aInfo : info) {
            if (aInfo.importance == ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND) {
                return aInfo.processName.equals(FUtils.getApp().getPackageName());
            }
        }
        return false;
    }
    ...
    static class ActivityLifecycleImpl implements ActivityLifecycleCallbacks {
        ...
        private static void fixSoftInputLeaks(final Activity activity) {
            ...
            InputMethodManager imm =
                    (InputMethodManager) FUtils.getApp().getSystemService(Context.INPUT_METHOD_SERVICE);
            if (imm == null) return;
